...
let description = {
    "Villageois":
        "Vous faites partie des Villageois et devez gagner avec eux. Vous n'avez pas de particularité.",

    "Loup-garou":
        "Vous faites partie des Loups-garou et devez gagner avec eux. Vous choisissez chaque nuit avec votre groupe un joueur à tuer. Vous pouvez parler entre loup via le -chatlg dans mes mp.",
    ...
    "Chasseur":
        "Vous faites partie des Villageois et devez gagner avec eux. Si vous mourrez vous obtiendrez un fusil et pourrez tuer une personne.",

    "Assassin":
        "Vous faites partie des Solitaires. Chaque nuit vous pouvez tuer une personne que vous souhaitez."
}
...
let vote_soso = "";
let soso_choix = "";
let soso_potion = {"mort": 1, "résurrection": 1};
let vote_chasseur = "";
...
function voteLG(playername) {
    Object.entries(rolesjoueurs).forEach(([key, value]) => {
        if (value.includes("Loup-Garou"))
            bot.whisper(key, `Votre groupe de loup a voté: ${playername}`);
    });
}
...
//      MP COMMANDS
//▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬▬
let WHITELISTED = ['PerrierBottle', 'ElFamosoClicker', 'Hqky'];
let ADMIN = ['PerrierBottle', 'ElFamosoClicker', 'Hqky'];
let BLACKLISTED = ['Kiyanor'];// uwu
...
bot.on('message', async (message) => {
    ...
    if (cmd.toString().toLowerCase().startsWith("-help")) {
        ...
        if (ADMIN.indexOf(pseudo) != -1) {
            ...
            vote_soso= "";
            soso_choix = "";
            soso_potion = {"mort": 1, "résurrection": 1};
        ...
        if (ADMIN.indexOf(pseudo) != -1) {
            ...
            await sleep(250);
            bot.chat("De plus le maire a deux votes");
            await sleep(250);
            bot.chat("au lieu d'un !");
            await sleep(250);
            bot.chat("Lors d'une action vous pouvez mettre 'aucun'");
            await sleep(250);
            bot.chat("si vous ne souhaitez tuer personne.");
            await sleep(250);
            bot.chat("---------- [  Bon Jeu   ] ----------");
        }
    }
        ...
        if (ADMIN.indexOf(pseudo) != -1) {
            ...
            while (true) {
                ...
                if (Object.values(rolesjoueurs).includes("Voyante")) {
                    bot.chat("[LG]: La voyante se reveille et choisit de voir les plus profonds désires d'un des joueurs.");
                    voyante_check = "";
                ...
                if (Object.values(rolesjoueurs).includes("Sorcière")) {
                    ...
                    bot.chat("/msg" + getKeyByValue(rolesjoueurs, "Sorcière") + "Vous avez" + soso_potion["mort"] + " potion de mort et " + soso_potion["résurrection"] + " potion de résurrection.");
                    soso_choix = "";
                    await waitForStringToBeNonEmpty(() => soso_choix);

                }
                if (Object.values(rolesjoueurs).includes("Assassin")) {
                    bot.chat("[LG]: Une pulsion meurtrière anime l'assassin.");
                    vote_as = "";
                ...
                await sleep(150);
                if (vote_soso.toLowerCase() != "aucun" && vote_soso != "") {
                    bot.chat("[LG]: " + vote_soso + " a été tué.");
                    bot.chat("[LG]: Il était " + rolesjoueurs[vote_soso]);
                    await sleep(150);
                    if (rolesjoueurs[vote_soso] === "Chasseur") {
                        can_vote_chasseur = !can_vote_chasseur;
                        bot.chat("[LG]: Le chasseur sort son fusil.");
                        bot.chat("/msg " + vote_soso + " Faite -ckill <pseudo> pour désigner qui tuer.");
                        bot.chat("/msg " + vote_soso + " Vous pouvez tuer personne en faisant -ckill aucun");
                        await waitForStringToBeNonEmpty(() => vote_chasseur);
                        if (vote_chasseur != "aucun") {
                            bot.chat("[LG]: " + vote_chasseur + " a été tué.");
                            let index = joueurs.indexOf(vote_chasseur);
                ...
                let index = joueurs.indexOf(vote_soso);
                    joueurs.splice(index, 1);
                }
                ...
                if (nb_jours == 1) {
                    ...
                    await sleep(250);
                    bot.chat("[LG]: Les élections pour le maire sont ouvertes !");
                    await sleep(250);
                    ...
                    await sleep(250);
                    bot.chat("[LG]: Vous ne pouvez voter que une seule fois !");
                    await sleep(250);
                    bot.chat("---------- [Vote du Maire] ----------");
                    await sleep(5000);
                    ...
                    can_vote_maire = !can_vote_maire;
                    attributionMaire();
                    bot.chat(`[LG]: Les éléctions sont finies. Le maire est ${maire}`);
                    await sleep(3000);
                }
        ...
        if (playing)
            bot.chat("/msg " + pseudo + " Vous ne pouvez pas vous rejoindre pendant une partie en cours.")
        else
            registerPlayer(pseudo);
    } else if (cmd.toString().toLowerCase().startsWith("-unregister")) {
        if (playing)
            bot.chat("/msg " + pseudo + " Vous ne pouvez pas vous quitter pendant une partie en cours.")
        else
            unregisterPlayer(pseudo);
    } else if (cmd.toString().toLowerCase().startsWith("-vote ")) {
        if (!playing)
            bot.chat("/msg " + pseudo + " Vous ne pouvez pas voter si la partie n'est pas démarrée.")
        else {
            if (joueurs.includes(pseudo)) {
                ...
                switch (rolesjoueurs[pseudo]) {
                    ...
                    case "Assassin":
                        ...
                        break
                    case "Sorcière":
                        if playervote.match(([a-zA-Z]+)\s([a-zA-Z0-9]*)) {
                            if (playervote.match[0] == "ressuciter" && soso_potion["résurrection"] > 0) {
                                if playervote.match[1] == vote_lg) {
                                    vote_lg = ""
                                }
                                if (playervote.match[1] == vote_as) {
                                    vote_as = ""
                                }
                                soso_choix = `ressuciter ${playervote.match[1]}`
                                soso_potion["résurrection"]--
                            }
                            if (playervote.match[0] == "tuer") {
                                vote_soso = playervote.match[1]
                                soso_choix = `ressuciter ${playervote.match[1]}`
                                soso_potion["mort"]--
                            }
                        }
                ...
                bot.chat("/msg " + pseudo + " Vous n'êtes pas dans la game actuelle.")
            }
        }
    } else if (cmd.toString().toLowerCase().startsWith("-votemaire")) {
        if (!playing)
            bot.chat("/msg " + pseudo + " Vous ne pouvez pas voter si la game n'est pas démarrée.")
        else {
            if (joueurs.includes(pseudo)) {
        ...
        if (!playing)
            ...
            if (joueurs.includes(pseudo)) {
                ...
                if (!can_vote_kill) {
                    bot.chat("/msg " + pseudo + " Les votes pour le pendu ne sont pas activés.");
                    return;
                }
                ...
                if (playervote != "aucun" && !joueurs.includes(playervote)) {
                    bot.chat("/msg " + pseudo + " Ce joueur est mort ou n'est pas dans la partie.");
                    return;
                }
        ...
        if (playing) {
            ...
            if (!couple.includes(pseudo)) {
                bot.chat("/msg " + pseudo + " Vous ne pouvez pas parler dans le tchat du couple.")
                return;
            }
            if (msg.length > 30) {
                bot.chat("/msg " + pseudo + " Ce messages est trop long.")
...
bot.on('message', (message) => {
    ...
    if (chat == "disconnect.spam")
        bot.chat("/msg Hqky I have been kicked for disconnect.spam");
});
